...
public class allEdges {
	...
	/* for the calculation of multiple maps astar
	 * what maps are on the path from the start node to end node
	 */
	public Stack<GeneralMap> getMaps(){
		...
		for (Edge e : mapEdges) {
			...
			graphMaps.get(e.getNode1().getMap().getMapId()).getNeighborM().add(graphMaps.get(e.getNode2().getMap().getMapId()));
			graphMaps.get(e.getNode2().getMap().getMapId()).getNeighborM().add(graphMaps.get(e.getNode1().getMap().getMapId()));
