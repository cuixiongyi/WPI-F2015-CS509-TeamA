...
public class DijkstraMultipleDestinations implements AlgoStrategy{
	...
	public Stack<Node> getRoute(allEdges alledges) {
		...
		while(i<endNodeId.length){ //last end node does not need to run Dijkstra on
		...
		order=getEndNodeOrder(); //top of the stack is the last node visited
								// it is the position of the node in endNodeId array +1
	...
	/* Calculate the order of the end nodes
	 * that would give the shortest total distance
	 * use the matrix as input
	 */
